...
def oink_points(player_score, opponent_score):
    ...
    ones=opponent_score%100-10*tens
    my_new=1
    my_new=max(2*tens-ones,1)
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    temp_dice=0
    if num_rolls==0:
        temp_dice=oink_points(player_score,opponent_score)
    else:
        temp_dice=roll_dice(num_rolls,dice)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
   ...
   #Lizzz RMK:single loop is to say the function only has one loop (one for or while...)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if(score0>score1):
        if last_leader==0:
            ...
            return 0 , 'Player 0 takes the lead by %d' %(score0-score1)
    elif(score0<score1):
        if last_leader==1:
            return 1, None
        else:
            return 1 , 'Player 1 takes the lead by %d' %(score1-score0)
    else:
        return None, None
    # END PROBLEM 6
